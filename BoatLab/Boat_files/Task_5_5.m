...
legend({'$\psi_{r}$', '$\psi + v$ (with $\hat{b}_{feedforward}$)', ...
    '$\psi + v$ (without $\hat{b}_{feedforward}$)'}, 'FontSize', 24,  ...
...
title(['Compass reference $\psi_{r} = 30$, measured compass course '...
    '$\psi + \psi_{w} + v$ and estimated course $\hat{\psi}$'], ... 
    'FontSize', 24, 'Interpreter', 'latex');
xlabel('t [s]', 'FontSize', 20); ylabel('Angle [deg]', 'FontSize', 20);
legend({'$\psi_{r}$', 'measured compass course ($\psi + \psi_{w} + v$)' ...
    , 'estimated compass course ($\hat{\psi}$)'}, 'FontSize', 24, ...
...
legend({'$\psi_{r}$', 'With Kalman filtered feedback', ...
    'Without Kalman filtered feedback'}, 'FontSize', 24, ...
    'Interpreter', 'latex', 'Location', 'best')
ax = gca; ax.FontSize = 24; grid on; axis([0 500 -5 40])
...
title('Rudder input $\delta$ and estimated bias $\hat{b}$', ...
    'FontSize', 24, 'Interpreter', 'latex');
xlabel('t [s]', 'FontSize', 20); ylabel('Angle [deg]', 'FontSize', 20);
legend({'$\hat{b}$', '$\delta$'}, 'FontSize', 24,  ...
    'Interpreter', 'latex', 'Location', 'best')
ax = gca; ax.FontSize = 24; grid on;
